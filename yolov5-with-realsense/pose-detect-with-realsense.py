...
def color_detect(color_image, box):
    ...
    # bgr_box = color_image[int(box[1]):int(box[3]), int(box[0]):int(box[2])]
...
def detect_with_realsense(pipeline):
    ...
    if len(det):
        ...
        for idx, (*box, conf, cls) in enumerate(reversed(det)):
            ...
            color_input = app.color_entry.get()

            if (class_input == names[c] == 'cup' and num_cup == 1) or (class_input == names[c] == 'bottle' and num_bottle == 1) or (class_input == names[c] == 'cup' and color_input == dominant_color):
    ...
    app.detect_num_var_1.set(num_bottle)
    app.detect_num_var_2.set(num_cup)
...
class RealSense_App:
    ...
    def create_gui(self):
        ...
        detect_info = tk.LabelFrame(info, text='Object Detection', font=('Times New Roman', 11, 'bold'), fg='gold', bg='gray20', bd=5, relief='groove')
        detect_info.grid(row=0, column=0, padx=5, pady=5)

        class_label = tk.Label(detect_info, text='Class: ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        class_label.grid(row=0, column=0, padx=5, pady=4, sticky='w')

        class_name_label = tk.Label(detect_info, text='Bottle', font=('Times New Roman', 9, 'bold'), fg='black', bg='white', bd=2, width=6, relief='groove')
        class_name_label.grid(row=0, column=1, padx=5, pady=4)

        class_name_label = tk.Label(detect_info, text='Cup', font=('Times New Roman', 9, 'bold'), fg='black', bg='white', bd=2, width=6, relief='groove')
        class_name_label.grid(row=0, column=2, padx=5, pady=4)

        detect_num_label = tk.Label(detect_info, text='#detections:', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        detect_num_label.grid(row=1, column=0, padx=5, pady=4, sticky='w')

        self.detect_num_var_1 = tk.StringVar()
        detect_num_var_1_label = tk.Label(detect_info, textvariable=self.detect_num_var_1, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', bd=2, width=6, relief='groove')
        detect_num_var_1_label.grid(row=1, column=1, padx=5, pady=4)

        self.detect_num_var_2 = tk.StringVar()
        detect_num_var_2_label = tk.Label(detect_info, textvariable=self.detect_num_var_2, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', bd=2, width=6, relief='groove')
        detect_num_var_2_label.grid(row=1, column=2, padx=5, pady=4)
        ...
        self.confidence_label = tk.Label(detect_info, text='Confidence:', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        self.confidence_label.grid(row=0, column=3, padx=20, pady=4, sticky='w')
        self.confidence_var = tk.StringVar()
        self.confidence_var_label = tk.Label(detect_info, textvariable=self.confidence_var, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', width=3, relief='groove', bd=2)
        self.confidence_var_label.grid(row=0, column=4, padx=4, pady=4, sticky='w')
        self.confidence_slider = ttk.Scale(detect_info, from_=0, to=100, name='confidence', orient=tk.HORIZONTAL, length=200, value=60, style='myStyle.Horizontal.TScale')
        self.confidence_slider.grid(row=0, column=5, padx=2, pady=4, sticky='w')

        self.iou_label = tk.Label(detect_info, text='IoU:', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        self.iou_label.grid(row=1, column=3, padx=20, pady=4, sticky='w')
        self.iou_var = tk.StringVar()
        self.iou_var_label = tk.Label(detect_info, textvariable=self.iou_var, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', width=3, relief='groove', bd=2)
        self.iou_var_label.grid(row=1, column=4, padx=4, pady=4, sticky='w')
        self.iou_slider = ttk.Scale(detect_info, from_=0, to=100, name='iou', orient=tk.HORIZONTAL, length=200, value=60, style='myStyle.Horizontal.TScale')
        self.iou_slider.grid(row=1, column=5, padx=2, pady=4, sticky='w')

        #-----------------------------------Object pose Frame-------------------------------------------------
        object_pose = tk.LabelFrame(info, text='Object Pose', font=('Times New Roman', 11, 'bold'), fg='gold', bg='gray20', bd=5, relief='groove')
        object_pose.grid(row=0, column=2, padx=5, pady=0)
        #------------------------------------------
        user_input = tk.Frame(object_pose, bg='gray20')
        user_input.grid(row=0, column=0, padx=2, pady=0)

        class_label = tk.Label(user_input, text='Enter class: ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        class_label.grid(row=0, column=0, padx=5, pady=4, sticky='w')
        
        self.class_entry = tk.Entry(user_input, font=('Times New Roman', 9, 'bold'), width=10, bd = 3)
        self.class_entry.grid(row=0, column=1, padx=10, pady=4, sticky='w')

        color_label = tk.Label(user_input, text='Color: ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        color_label.grid(row=0, column=2, padx=5, pady=4, sticky='w')

        self.color_entry = tk.Entry(user_input, font=('Times New Roman', 9, 'bold'), width=10, bd = 3)
        self.color_entry.grid(row=0, column=3, padx=8, pady=4, sticky='w')

        empty_label = tk.Label(user_input, text='___', font=('Times New Roman', 9, 'bold'), fg='gray20', bg='gray20')
        empty_label.grid(row=0, column=4, padx=5, pady=4, sticky='w')

        clear_button = tk.Button(user_input, text="Clear", font=('Times New Roman', 9, 'bold'), height=1, width=7, relief='groove', bd=2, command=self.clear_pose)
        clear_button.grid(row=0, column=5, padx=3, pady=4, sticky='e')

        #----------------------------------------
        pose = tk.Frame(object_pose, bg='gray20')
        pose.grid(row=1, column=0, padx=5, pady=1)

        pose_label = tk.Label(pose, text='Pose (in cm): ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        pose_label.grid(row=0, column=0, padx=4, pady=5, sticky='w')
        X_label = tk.Label(pose, text='X = ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        X_label.grid(row=0, column=1, padx=5, pady=5, sticky='w')

        self.X_var = tk.StringVar()
        X_var_label = tk.Label(pose, textvariable=self.X_var, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', width=8, relief='groove', bd=3)
        X_var_label.grid(row=0, column=2, padx=5, pady=5, sticky='w')

        Y_label = tk.Label(pose, text='Y = ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        Y_label.grid(row=0, column=3, padx=5, pady=5, sticky='w')

        self.Y_var = tk.StringVar()
        Y_var_label = tk.Label(pose, textvariable=self.Y_var, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', width=8, relief='groove', bd=3)
        Y_var_label.grid(row=0, column=4, padx=5, pady=5, sticky='w')

        Z_label = tk.Label(pose, text='Z = ', font=('Times New Roman', 9, 'bold'), fg='white', bg='gray20')
        Z_label.grid(row=0, column=5, padx=5, pady=5, sticky='w')

        self.Z_var = tk.StringVar()
        Z_var_label = tk.Label(pose, textvariable=self.Z_var, font=('Times New Roman', 9, 'bold'), fg='black', bg='white', width=8, relief='groove', bd=3)
